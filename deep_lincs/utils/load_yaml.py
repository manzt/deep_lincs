...
def create_dataset(yaml_file, only_landmark=True, **filter_kwargs):
    ...
    return Dataset.from_dataframes(data, sample_metadata.reindex(data.index), gene_metadata)
...
def parse_settings(yaml_file):
    ...
    sample_meta_loader = MetaDataLoader.from_node(d["sample_metadata"], data_dir)
    gene_meta_loader = MetaDataLoader.from_node(d["gene_metadata"], data_dir)
