...
class Dataset:
    ...
    Offers methods to subset, filter, normalize, sample 
    and prepare L1000 data for analysis.

    Parameters
    ----------
    data : dataframe, shape (n_samples, n_features)
            A `sample` by `gene` expression matrix 
            padded to the right with per sample metadata.
    ...
    def filter_rows(self, **kwargs):
        ...
        kwargs :
                ...
                Each kwarg must follow the following.
                    `keyword` - a column in metadata
                    `arg`     - a list of values to filter from keyword field
    ...
    def normalize_by_gene(self, normalizer="standard_scale"):
        ...
        normalizer: str or func (optional, default 'standard_scale')
                ...
                and 'z_score'. If a function is provided, it must take one argument (array),
    ...
    def train_val_test_split(self, p1=0.2, p2=0.2):
        ...
        Returns
        -------
        train, val, test : tuple of Datasets
        """
