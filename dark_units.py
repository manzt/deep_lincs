...
n_replicates = 4
h_sizes = [2 ** n for n in range(1, 9)]  # 2, 4, ..., 256
...
out_dir = "../data/dark_unit_analysis"
...
if __name__ == "__main__":
    ...
    # Tensorboard stuff
    logdir = os.path.join(
        "logs", f"darkunits_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    )
    ...
    for h_size in h_sizes:
        for rep in range(n_replicates):
            ...
            df.sample(recorded_samples).to_csv(
                os.path.join(out_dir, f"hidden_out/{h_size}_{rep}.csv"), index=False
            )
    ...
    with open(os.path.join(out_dir, "dark_unit_summary.csv"), "w") as f:
